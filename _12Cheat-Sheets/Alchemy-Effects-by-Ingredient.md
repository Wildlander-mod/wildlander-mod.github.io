...
.alchemy-effects-table table tbody tr:hover {
...
function filterAlchemyEffectsTable() {
  ...
  rows.forEach(row => {
    ...
    const searchMatch = ingredient.includes(searchTerm) || effect.toLowerCase().includes(searchTerm) || description.includes(searchTerm);
